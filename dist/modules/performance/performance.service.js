...
let PerformanceService = class PerformanceService {
    ...
    async listTeacherAssessments(teacherId, query) {
    ...
    async createAssessment(teacherId, payload) {
        ...
        if (!resolvedTermId) {
            throw new common_1.NotFoundException('No active academic term available for this section');
        }
    ...
    async upsertMarks(teacherId, payload) {
        ...
        const assignment = await this.ensureTeacherAssignment(teacherId, assessment.subjectOffering?.sectionId ?? undefined, assessment.subjectId);
        if (assessment.subjectOfferingId && assignment.subjectOfferingId && assessment.subjectOfferingId !== assignment.subjectOfferingId) {
            throw new common_1.ForbiddenException('You are not assigned to this assessment offering');
        }
        for (const record of payload.records) {
            if (record.marksObtained > Number(assessment.maxMarks)) {
                throw new common_1.BadRequestException(`Marks cannot exceed ${Number(assessment.maxMarks)} for this assessment`);
            }
        }
        await this.prisma.$transaction(payload.records.map((record) => this.prisma.mark.upsert({
            where: {
    ...
    async createStudentRemark(teacherId, payload) {
        const student = await this.prisma.student.findUnique({
        ...
        if (!assignment) {
            throw new common_1.ForbiddenException('You are not assigned to this student section');
        }
        const resolvedTermId = payload.termId ??
    ...
    async resolveActiveTermId(sectionId) {
        ...
        return (await this.prisma.academicTerm.findFirst({
            where: {
    ...
    async ensureTeacherAssignment(teacherId, sectionId, subjectId) {
        const where = {
            teacherId,
            subjectId,
            sectionId,
    ...
    async recalculateFinalInternalMarks(assessmentId) {
        const assessment = await this.prisma.assessment.findUnique({
        ...
        const policyTotal = Number(assessment.subjectOffering.internalMarksPolicy?.totalMarks ?? assessment.subjectOffering.totalInternalMarks);
